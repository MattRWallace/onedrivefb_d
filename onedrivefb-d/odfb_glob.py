...
* When there is network issue at an API call, the calling thread is put to sleep
  and thread manager will wake it up when the network seems fine. When the
  caller is waken up, it will retry the function that failed before.
...
def will_update_last_run_time():
    global update_last_run_timestamp
...
class ConfigSet:
    ...
    params = {
        ...
        'ONEDRIVEFB_ROOT_PATH': None,
        'ONEDRIVEFB_TOKENS': None,
        'ONEDRIVEFB_TOKENS_EXP': None,
        'USE_GUI': False,
        'MIN_LOG_LEVEL': logging.DEBUG,
        'LOG_FILE_PATH': '/var/log/onedrivefb_d.log',
    ...
    def __init__(self, setup_mode=False):
        # no locking is necessary because the code is run before multithreading
        if not ConfigSet.initialized:
            self.initialize()

        if not setup_mode:
            if os.path.exists(ConfigSet.APP_IGNORE_FILE):
                self.ignore_list = odfb_ignore_list.IgnoreList(
                    ConfigSet.APP_IGNORE_FILE,
                    ConfigSet.params['ONEDRIVEFB_ROOT_PATH'])
            else:
                ConfigSet.logger.info('ignore list file was not found.')
                ConfigSet.ignore_list = None

    def initialize(self, setup_mode=False):
        ...
        ConfigSet.APP_CONF_PATH = ConfigSet.OS_HOME_PATH + '/.onedrivefb'
        if not os.path.exists(ConfigSet.APP_CONF_PATH):
            get_logger().critical('onedrivefb-d may not be installed properly.'
                                  ' Exit.')
            sys.exit(1)
        ...
        if os.path.exists(ConfigSet.APP_CONF_FILE):
            try:
                ...
                get_logger().info(
                    'fail to read config file "' + ConfigSet.APP_CONF_FILE +
                    '". Use default.')
        elif not setup_mode:
            get_logger().critical('onedrivefb-d config file does not exist. Exit.')
            sys.exit(1)
        if ConfigSet.params['ONEDRIVEFB_ROOT_PATH'] is None and not setup_mode:
            get_logger().critical('path to local OneDriveFB repo is not set.')
            sys.exit(1)
        ConfigSet.LAST_RUN_TIMESTAMP = \
            str_to_time(ConfigSet.params['LAST_RUN_TIMESTAMP'])
    ...
    def set_root_path(self, path):
        ConfigSet.params['ONEDRIVEFB_ROOT_PATH'] = path
    ...
    def get_access_token(self):
        if ConfigSet.params['ONEDRIVEFB_TOKENS'] is not None:
            return ConfigSet.params['ONEDRIVEFB_TOKENS']
        else:
            return None

    def is_token_expired(self):
        return str_to_time(ConfigSet.params['ONEDRIVEFB_TOKENS_EXP']) < now()

    def set_access_token(self, token):
        d = now() + timedelta(seconds=int(token['expires_in']))
        ConfigSet.params['ONEDRIVEFB_TOKENS'] = token
        ConfigSet.params['ONEDRIVEFB_TOKENS_EXP'] = time_to_str(d)
        ConfigSet.is_dirty = True
    ...
    def dump(self):
        try:
            ...
            get_logger().warning(
                'failed to dump config to file "' +
                ConfigSet.APP_CONF_FILE + '".')
