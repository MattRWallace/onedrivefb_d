...
do_clean() {
	sudo rm -rf temp onedrivefb_d/temp build onedrivefb_d/build dist onedrivefb_d/dist *.egg-info \
	    onedrivefb_d/*.egg-info setup.cfg onedrivefb_d/setup.cfg onedrivefb_d/__pycache__
}
...
rm -rf ~/.onedrivefb
mkdir ~/.onedrivefb
cp ./onedrivefb_d/res/default_ignore.ini ~/.onedrivefb/ignore_v2.ini

sudo touch /var/log/onedrivefb_d.log
sudo chown $CURRENT_USER /var/log/onedrivefb_d.log

echo -e "\e[92monedrivefb-d installed successfully.\e[0m"
echo -e "\e[92mPlease run command \`onedrivefb-pref\` to set up the program.\e[0m"
